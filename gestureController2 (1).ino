...
void setup() {
 ...
 Serial.begin(115200);
  ...
  pinMode(slaveout1,OUTPUT);
  pinMode(slaveout2,OUTPUT);
  digitalWrite(slaveout1,LOW);
  digitalWrite(slaveout2,LOW);
...
void readIR(uint8_t times){
  ...
  if(distance_TL != 0 || distance_TR != 0 || distance_BL != 0 || distance_BR != 0 || distance_L != 0 || distance_R != 0){
    distance_TR = map(constrain(distance_TR, 0, 1023), 0, 1023, 0, 255);
    distance_TL = map(constrain(distance_TL, 0, 1023), 0, 1023, 0, 255);
    distance_BR = map(constrain(distance_BR, 0, 1023), 0, 1023, 0, 255);
    distance_BL = map(constrain(distance_BL, 0, 1023), 0, 1023, 0, 255);
    distance_L = map(constrain(distance_L, 0, 1023), 0, 1023, 0, 255);
    distance_R = map(constrain(distance_R, 0, 1023), 0, 1023, 0, 255);
...
void determineGesture(){
  if(distanceArrayCounter > 2){       // If the arrays have more than 2 elements then determine the gesture
    ...
    for(uint8_t x = 1; x < distanceArrayCounter; x++){
      ...
      if(distancePost_L > 30 && distancePostNeg_L < 30){
        increasing_L = true;
        decreasing_L = false;
      }
      else if(distancePostNeg_L > 30){
        increasing_L = false;
        decreasing_L = true;
      }
      if(distancePost_R > 30 && distancePostNeg_R < 30){
        increasing_BR = true;
        decreasing_BR = false;
      }
      else if(distancePostNeg_R > 30){
        increasing_R = false;
        decreasing_R = true;
      }
      ...
      switch(expectedGesture){
        case 0:                   // If "expectedGesture" is 0 then determine the expected gesture 
          ...
          digitalWrite(slaveout1,LOW);
          digitalWrite(slaveout2,LOW);
          if(increasing_TL && increasing_BL && increasing_L && !increasing_R && !increasing_TR && !increasing_BR){         // Left to right movement
            expectedGesture = 1;  // Expected an "east" gesture
          }
          else if(increasing_TR && increasing_BR && increasing_R && !increasing_L && !increasing_TL && !increasing_BL){    // Right to left movement
        ...
        case 1:   
          ...
          if(increasing_TR && increasing_BR && increasing_R && decreasing_L && decreasing_TL && decreasing_BL){
        ...
        case 2:
          if(increasing_TL && increasing_BL && increasing_L && decreasing_R && decreasing_BR && decreasing_TR){
            if(readGesture){
              Serial.println("WEST");
              gesture = 2;
              holdMillis = 300;
              lastHoldMillis = millis();
              digitalWrite(slaveout2,HIGH);
            }
            expectedGesture = 0;
            lastDistanceHold_TR = distance_TR;
            lastDistanceHold_BR = distance_BR;
